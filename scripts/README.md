...
3. [Resource Setup Walkthrough](#resource-setup-walkthrough)
...
1. Creating and loading of sample datasets on SQL Data Warehouse and Hive.
...
1. **Data Source:** This tutorial uses the **FactInternetSale** table from the **AdventureWorks** Data. _Instructions to load this data described below_.  
1. **Polybase support/activities:** On-Prem SQL Server will be simulated using Azure SQL Dataware. There is no Polybase support (at the time of writing this) 
for Azure SQL Database and SQL Servers prior to 2016. No different actions or commands are needed to achieve the same effect on On-Premises SQL Server **2016** Polybase
; as you would using Polybase on Azure SQL Datawarehouse. 
...
1. An existing Storage Account with Read/Write access.  
	- This will hold more rows of the **FactInternetSales** sample data from the **AdventureWorks** datasets.



## Resource Setup Walkthrough
...
(mapped to HDI cluster as default filesystem), a SQL Data Warehouse with Polybase support and sample db and table attached to the DW.  
...
- Use your existing Storage Account to save some extra sample data. Copy the extra sample data from your local machine data to Azure Storage blob with [AzCopy tool](https://azure.microsoft.com/en-us/documentation/articles/storage-use-azcopy/).   
Follow **ONLY** [Step 1](https://azure.microsoft.com/en-us/documentation/articles/sql-data-warehouse-get-started-load-with-azure-data-factory/) to copy the sample data (_three more years of **FactInternetSale** data_) from your local machine to Azure Storage Blob. 
...
### Script Contents
1. Azure Data Factory configuration
1. Azure HDInsight configuration
1. Azure SQL DB (simulating On-Premises DB) configuration
1. Azure Storage Accounts configuration


Interacting with the Hive environment on HDI can be done via **Ambari Views** or **ssh**, both have same syntax and effect. For this tutorial we will
use the Ambari View.  
...
![Legacy cardinality Estimator](Markdown\media\legacy-estimation.PNG "Legacy cardinality")  
...
![Product Table Schema](Markdown\media\product-schema-scaled.PNG "Product Table")  
